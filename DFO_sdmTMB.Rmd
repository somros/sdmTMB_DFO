...
1. There are not as many data points for the DFO data, also because different areas (Queen Charlotte Sound, Hecate Strait, and Haida Gwaii) are surveyed in different years. Using all data alleviates this, but it may make more sense to aggregate all data and run a model without temporal definition, rather than making predictions year-by-year where some areas are not sampled at all in one year. This was not the case for the RACE data, because although the stations vary in each year, they span most of the GOA in most years (with a couple of exceptions here and there). This raises the argument of whether we want to do the same for the RACE data in the rest of the GOA.
2. Number of individuals is the "Estimated number of pieces in the catch", expressed as Catch Count (pieces). A lot of sets (the DFO equivalent of tows) have NA for that. So it seems like CPUE in weight is more reliable than CPUE in individuals. This is a mismatch with the GOA RACE data, where we decided to do by numbers (because that is what Atlantis needs). Not sure how to bring things together.
...
m_depth <- sdmTMB(
  data = dfo_data, 
  formula = biom_kgkm2 ~ 0 + s(depth, k = 5) + as.factor(year), # predicting numbers - we will need numbers for Atlantis
  time = "year", 
  spde = dfo_spde, 
  reml = TRUE,
  anisotropy = FALSE,
...
**Important:** depth in the DFO data is a positive number. Depth in the prediction grid we obtained from the ETOPO rasters is a negative number. When we use depth as predictor for in our regular grid, make sure depth is a positive number for consistency with the model variable, or else everything will be upside-down.
```{r}
...
See estimates per box for all years. Silence boundary boxes as they throw the scale out of whack (and they do not need predictions). Plot BC part of the model domain only.
```{r, fig.width = 12, fig.height = 15}
...
```{r, fig.width = 6, fig.height = 4}
ggplot(data = dfo_data, aes(depth))+
...
means_all_years <- predictions_by_box %>% group_by(box_id, area, boundary) %>% summarise(all_years_biomkgkm2 = mean(mean_estimates))

ggplot()+
  geom_sf(data = means_all_years[means_all_years$box_id>=92,], aes(fill = log1p(all_years_biomkgkm2)))+ # log for visualisation
  scale_fill_viridis(name = expression(paste("Log(CPUE) kg ", km^-2)))+
  geom_sf(data = coast)+
  coord_sf(xlim = c(coord_lims$xmin,coord_lims$xmax), ylim=c(coord_lims$ymin,coord_lims$ymax))+
  theme_minimal()+
  labs(title = paste(dfo_data$name, "mean predicted CPUE by Atlantis box (2003-2019)"))#, race_data$stage, sep = " "))
```
Plot data from all years together.
```{r, fig.width = 10, fig.height = 5}
ggplot()+
  geom_sf(data = dfo_sf, aes(colour = log1p(biom_kgkm2)), size = 2, alpha = 0.5)+
  scale_colour_viridis_c()+
  geom_sf(data = coast)+
  coord_sf(xlim = c(coord_lims$xmin,coord_lims$xmax), ylim=c(coord_lims$ymin,coord_lims$ymax))+
  theme_minimal()+
  labs(title = paste(this_group,"CPUE from DFO bottom trawl survey", sep = " "))
```
...
dfo_grid_tmp <- dfo_data %>% dplyr::select(lon, lat, depth)
...
dfo_corr <- dfo_data %>% left_join(dfo_grid, by = c("year", "lat", "lon"))
```
...
Same as RACE data, zeroes are overestimated.
...
```{r}
means_all_years <- means_all_years %>% mutate(biomass = all_years_biomkgkm2*area*1e-06)
...
```{r}
means_canada %>% select(box_id, all_years_biomkgkm2, biomass) %>% st_set_geometry(NULL) %>% kable(align = 'lccc', format = "markdown", 
